...
export default function HotelRoomPlannerPage() {
  ...
  const [
    selectedRoom,
    setSelectedRoom,
  ] = useState<PlannerRoom | null>(null);
  ...
  async function handleRoomStatusUpdate(
    room: PlannerRoom,
    roomStatus: string
  ) {
    if (!membership || saving) return;
    ...
    try {
      await updateRoomOperationalStatus(
        membership.company_id,
        room.id,
        {
          roomStatus,
      ...
      setSuccessMessage(
        `Oda ${room.room_number} durumu güncellendi.`
      );
    } catch (error: unknown) {
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Oda durumu güncellenemedi."
      );
    } finally {
      setSaving(false);
    }
  ...
  async function handleHousekeepingUpdate(
    room: PlannerRoom,
    ...
    try {
      ...
      setSuccessMessage(
        `Oda ${room.room_number} temizlik durumu güncellendi.`
      );
    } catch (error: unknown) {
      setErrorMessage(
        error instanceof Error
          ? error.message
          : "Temizlik durumu güncellenemedi."
      );
    } finally {
      setSaving(false);
    }
                      ...
                      <div className="sticky left-0 z-10 min-h-24 border-r border-white/10 bg-slate-900 p-4">
                        <div className="flex items-start justify-between gap-3">
                          <button
                            ...
                            className="text-left"
                          >
                            <p className="text-xl font-black transition hover:text-orange-400">
                              Oda{" "}
                              {
                                room.room_number
                              }
          ...
          <aside className="relative z-10 h-full w-full max-w-md overflow-y-auto border-l border-white/10 bg-slate-950 p-6 shadow-2xl">
            ...
            <section className="mt-7 rounded-3xl border border-white/10 bg-white/[0.03] p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-3">
                <span
                  className={`rounded-full px-4 py-2 text-xs font-black ${housekeepingClasses(
                ...
                <span className="rounded-full bg-white/5 px-4 py-2 text-xs font-black text-slate-300">
                  {roomStatusLabel(
            ...
            <section className="mt-6">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Kat hizmetleri durumunu anında güncelleyin.
              </p>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                {[
                  ...
                  <button
                    ...
                    disabled={saving}
                    onClick={() =>
                      void handleHousekeepingUpdate(
                        selectedRoom,
                        item.value
                      )
                    }
                    className={`min-h-14 rounded-2xl border px-4 font-black transition disabled:opacity-50 ${
                      selectedRoom.housekeeping_status ===
                      item.value
            ...
            <section className="mt-7">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Odanın satışa ve kullanıma uygunluğunu yönetin.
              </p>
            ...
            <div className="mt-7 rounded-2xl border border-amber-500/20 bg-amber-500/10 p-4 text-sm text-amber-300">
              Bakımda veya kullanım dışı yapılan odalara yeni rezervasyon atanamaz.
            </div>
          </aside>
        </div>
      )}
